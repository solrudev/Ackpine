 ...
 * {@code ru.solrudev.ackpine.compress.zip.archivers.compress.ZipArchiveEntry} instead of {@link java.util.zip.ZipEntry}.
 * </p>
 * <p>
 ...
 * <li>getEntries and getEntry return {@code ru.solrudev.ackpine.compress.archivers.zip.ZipArchiveEntry} instances.</li>
...
public class ZipFile implements Closeable {
	 ...
	 * @since 1.1
	 */
	private static final Comparator<ZipArchiveEntry> offsetComparator = (entry1, entry2) -> {
		int result = Long.compare(entry1.getDiskNumberStart(), entry2.getDiskNumberStart());
		if (result == 0) {
			result = Long.compare(entry1.getLocalHeaderOffset(), entry2.getLocalHeaderOffset());
		}
		return result;
	};
...
//			.thenComparingLong(ZipArchiveEntry::getLocalHeaderOffset);
	...
	private static boolean positionAtEndOfCentralDirectoryRecord(final FileChannel channel) throws IOException {
		...
		if (position > ZIP64_EOCDL_LENGTH) {
			...
			wordBuf.flip();
			found64 = wordBuf.equals(ByteBuffer.wrap(ZIP64_EOCD_LOC_SIG));
			if (!found64) {
...
//			final LinkedList<ZipArchiveEntry> entriesOfThatName = nameMap.computeIfAbsent(name, k -> new LinkedList<>());
			...
			if ((entriesOfThatName = nameMap.get(name)) == null) {
				LinkedList<ZipArchiveEntry> newValue = new LinkedList<>();
				nameMap.put(name, newValue);
				entriesOfThatName = newValue;
			}
	...
	private void positionAtCentralDirectory64() throws IOException {
		...
		wordBbuf.rewind();
		IOUtils.readFully(archive, wordBbuf);
		if (!Arrays.equals(wordBuf, ZIP64_EOCD_SIG)) {
	...
	private boolean startsWithLocalFileHeader() throws IOException {
		...
		wordBbuf.rewind();
		IOUtils.readFully(archive, wordBbuf);
		return Arrays.equals(wordBuf, LFH_SIG);
	}
}