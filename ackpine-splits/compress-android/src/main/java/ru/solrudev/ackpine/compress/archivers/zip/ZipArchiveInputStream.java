...
import static ru.solrudev.ackpine.compress.archivers.zip.ZipConstants.DWORD;
import static ru.solrudev.ackpine.compress.archivers.zip.ZipConstants.SHORT;
import static ru.solrudev.ackpine.compress.archivers.zip.ZipConstants.WORD;
import static ru.solrudev.ackpine.compress.archivers.zip.ZipConstants.ZIP64_MAGIC;
...
public class ZipArchiveInputStream extends ArchiveInputStream<ZipArchiveEntry> implements InputStreamStatistics {
    ...
    public static boolean matches(final byte[] signature, final int length) {
        if (length < ZipFile.LFH_SIG.length) {
            return false;
        }
    ...
    private final ByteBuffer buf = ByteBuffer.allocate(ZipFile.BUFFER_SIZE);
    ...
    private void closeEntry() throws IOException {
        ...
        if (currentEntryHasOutstandingBytes()) {
            ...
            final long inB = current.entry.getMethod() == ZipEntry.DEFLATED ? getBytesInflated() : current.bytesRead;
    ...
    private boolean findEocdRecord() throws IOException {
        ...
        while (skipReadCall || (currentByte = readOneByte()) > -1) {
            ...
            if (currentByte != ZipFile.EOCD_SIG[1]) {
                if (currentByte == -1) {
                    break;
                }
            ...
            currentByte = readOneByte();
            if (currentByte != ZipFile.EOCD_SIG[2]) {
                if (currentByte == -1) {
                    break;
                }
                skipReadCall = isFirstByteOfEocdSig(currentByte);
            ...
            if (currentByte == ZipFile.EOCD_SIG[3]) {
                return true;
            }
            skipReadCall = isFirstByteOfEocdSig(currentByte);
        }
        ...
        try {
            READ_LOOP: for (int i = 0; ; ) {
                for (int j = 0; i <= PREAMBLE_GARBAGE_MAX_SIZE - 4 && j <= header.length - 4; ++j, ++i) {
                    ...
                    if (sig.equals(new ZipLong(ZipFile.EOCD_SIG))) {
                        // empty archive:
    ...
    private void readStoredEntry() throws IOException {
        ...
        while (!done) {
            final int r = in.read(buf.array(), off, ZipFile.BUFFER_SIZE - off);
            if (r <= 0) {
